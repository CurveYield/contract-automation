import { exactKeys,integer,fullName,commitSha,boundedString,frozenClone,sha256,fail } from '../../../packages/audit-github-direct-protocol/src/index.mjs';
import { buildPublicationLedgerPath } from '../../../packages/audit-github-direct-ledger/src/index.mjs';
const API_BASE='https://api.github.com',CONTROL_BRANCH='audit-direct/control-v1',ABSENT_BLOB_SHA='0'.repeat(40);
const fingerprint=(content)=>sha256(content).slice(7,47),encodePath=(path)=>path.split('/').map(encodeURIComponent).join('/'),encodeContent=(value)=>Buffer.from(JSON.stringify(value),'utf8').toString('base64');
function decodeResponseContent(value,path){const text=Buffer.from(value,'base64').toString('utf8');return path.startsWith('.audit-direct/v1/')?JSON.parse(text):text;}
...
async function createSideEffect(api,plan,issueNumber){
  if(plan.kind==='check')return api.request('POST',`${api.repoPath(plan)}/check-runs`,{name:plan.name,head_sha:plan.targetCommitSha,status:'completed',conclusion:plan.conclusion,external_id:plan.idempotencyKey,output:{title:plan.name,summary:plan.summary}});
  if(plan.kind==='status')return api.request('POST',`${api.repoPath(plan)}/statuses/${plan.targetCommitSha}`,{state:plan.state,description:plan.description,context:plan.context});
  return api.request('POST',`${api.repoPath(plan)}/issues/${issueNumber}/comments`,{body:`${plan.body}\n\n${commentMarker(plan)}`});
}
export function createGitHubActionsTransport(input){
  ...
  const publicationPath=(value)=>buildPublicationLedgerPath({jobId:value.jobId,publicationId:value.publicationId});
