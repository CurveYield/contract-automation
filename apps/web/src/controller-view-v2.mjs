import { controllerDetailModelV1 } from './controller-detail-model-v1.mjs';
...
function ensureDetailStyles() {
  if (typeof document === 'undefined' || document.getElementById('controller-detail-styles')) return;
  const link = document.createElement('link');
  link.id = 'controller-detail-styles';
  link.rel = 'stylesheet';
  link.href = './controller-detail-v1.css';
...
function renderRows(bodyId, rows, columns) {
  ...
  if (rows.length === 0) {
    ...
    for (const item of rows) {
      const row = document.createElement('tr');
      for (const column of columns) row.append(createElement('td', String(column(item) ?? '—')));
      rendered.push(row);
    }
  }
...
export function assertTier3BrowserCompatibilityV1(value) {
  ...
  if (value.adapterVersion !== 'tier3-controller-adapter-v1') throw new TypeError('This browser release is incompatible with the controller adapter.');
...
function activeView(value) {
  ...
  if (!projection || projection.schemaVersion !== 'hosted-operator-state-v1') throw new TypeError('Active controller pointer has no compatible hosted projection.');
  ...
  const assignmentEvidence = (projection.assignments ?? []).reduce((total, assignment) => total + (assignment.submission?.evidenceRefCount ?? 0), 0);
