...
function resetCampaign() {
  for (const id of ['campaign-status', 'completion-status', 'security-verdict', 'controller-state-commit']) text(id, '—');
  for (const id of [
...
function renderCampaignEnvelope(value) {
  ...
  text('controller-state-commit', value.controllerStateCommit);
...
byId('controller-connect').addEventListener('click', async () => {
  try {
    status('Checking immutable controller protocol compatibility…');
    const compatibility = await apiClient().getControllerCompatibility();
    renderCompatibility(compatibility);
    status('Tier 3 controller protocol is compatible. Live state reads resolve to an exact Git commit.');
  } catch (error) {
...
byId('load-campaign').addEventListener('click', async () => {
  ...
  try {
    status(`Resolving authoritative state for ${campaignId}…`);
    const envelope = await apiClient().getControllerCampaign(campaignId);
    renderCampaignEnvelope(envelope);
    status(`Loaded ${campaignId} at exact controller state commit ${envelope.controllerStateCommit}.`);
  } catch (error) {
    status(`${error.code ? `${error.code}: ` : ''}${error.message}`, true);
  }
});
