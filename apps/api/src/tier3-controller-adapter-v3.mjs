...
function headers(token) {
  return {
    ...
    'user-agent': 'CurveYield-Tier3-Controller-Adapter-v3',
...
async function resolveStateCommit(config, fetcher) {
  const response = await fetcher(`https://api.github.com/repos/${CONTROLLER_REPOSITORY}/commits/${encodeURIComponent(config.stateRef)}`, {
...
export async function controllerCompatibilityResponseV3(env) {
  const config = configuration(env);
  if (!config) return json({ error: { code: 'controller_not_configured', message: 'Tier 3 controller adapter v3 is not fully configured' } }, 503);
  return json({
    ...
    mutationMode: 'disabled-until-session-bound-actor-auth-v3'
  });
}

export async function controllerProjectionResponseV3(campaignId, env, fetcher = fetch) {
  ...
  let response;
  try {
    response = await fetcher(`https://api.github.com/repos/${CONTROLLER_REPOSITORY}/contents/hosted-projections/v3/${campaignId}.json?ref=${stateCommit}`, {
      method: 'GET', headers: headers(config.token)
    });
  } catch {
