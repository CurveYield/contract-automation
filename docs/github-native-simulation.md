...
All external fork traffic passes through the fail-closed policy documented in [`rpc-method-policy-v2.md`](rpc-method-policy-v2.md). Only listed methods may reach the selected `RPC_*` secret.

The proxy validates a complete request or batch before local metadata, deterministic-account handling, cache lookup, retry, or upstream forwarding. When one method is unsupported:

- it returns JSON-RPC code `-32601` with application code `CALL_NOT_SUPPORTED`;
