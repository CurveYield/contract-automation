...
- DNS routes for `api.preflight.curveyield.online` and `preflight.curveyield.online`.
...
Use a GitHub token restricted to `CurveYield/contract-automation` with Actions write permission. The client and runner API keys must be different long random values.

Configure R2 browser-upload CORS to allow `PUT` from `https://preflight.curveyield.online` with the `content-type` header. Configure an R2 lifecycle rule that deletes `uploads/` and `jobs/` objects after 30 days.
...
PAGES_PROJECT_NAME=contract-automation-preflight
```
