...
test('Cloudflare-orchestrated simulation uses optional archive pools and configured engine', async () => {
  const statuses = [];
  let published;
  ...
  const apiClient = {
    ...
    async publishResult(_jobId, result, report) {
    ...
    services: {
      ...
      startForkEngine: async ({ mode, forkUrl, block }) => {
        ...
        return {
          ...
          runtime: {
            async execute(step) { return { blocks: step.blocks }; }
          },
