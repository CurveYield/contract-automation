...
test('build publishes Tier 3 at root and byte-preserves accepted Lite under execution', () => {
  ...
  for (const relative of filesBelow(publicDir)) {
    sameFile(path.join(publicDir, relative), path.join(executionDir, relative));
  }
  for (const relative of filesBelow(tier3Dir)) {
    sameFile(path.join(tier3Dir, relative), path.join(output, relative));
  }
  ...
  sameFile(path.join(root, 'apps/web/src/controller-view-v2.mjs'), path.join(output, 'controller-view.js'));
  sameFile(path.join(root, 'apps/web/src/controller-detail-model-v1.mjs'), path.join(output, 'controller-detail-model-v1.mjs'));
  ...
  assert.match(rootHtml, /href="\.\/execution\/"/);
