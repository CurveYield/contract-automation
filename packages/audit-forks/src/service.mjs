...
import { deleteForkOperation, exportCheckpointOperation, publishCheckpointOperation, readCheckpointOperation, restoreCheckpointOperation } from './checkpoint-operations.mjs';
...
export class ForkService {
  ...
  async upsertTenantForkState(state) {
    ...
    const key = tenantForkIndexKey(state.tenantId);
    for (let attempt = 0; attempt < 4; attempt += 1) {
      const record = await this.storage.get(key);
      const index = record ? parseRecord(record) : {
        schemaVersion: 'tenant-fork-index-v1', tenantId: state.tenantId, entries: [], updatedAt: state.updatedAt
      };
      const entry = {
        forkId: state.forkId,
  ...
  async createFork(input) {
    ...
    try {
      await this.storage.put(forkCurrentKey(request.forkId), canonicalJson(requestedState), { onlyIf: { etagDoesNotMatch: '*' } });
    } catch (cause) {
      if (!(cause instanceof ConditionalWriteError || cause?.code === 'precondition_failed')) throw cause;
    }
  ...
  async transitionFork(input) {
    ...
    let written;
    try {
      written = await this.storage.put(forkCurrentKey(input.forkId), canonicalJson(next), { onlyIf: { etagMatches: current.etag } });
    } catch (cause) {
      if (cause instanceof ConditionalWriteError || cause?.code === 'precondition_failed') throw new ForkStateError('stale_state', 'Current state ETag changed');
      throw cause;
    }
