...
export class ForkService {
  ...
  operationTrace() {
    return this.storage.operationTrace();
  }

  clearOperationTrace() {
    this.storage.clearOperationTrace();
  }
  ...
  async #upsertTenantForkState(state) {
    ...
    if (
      ...
      request.attemptId !== state.attemptId ||
      request.requestDigest !== state.requestDigest
    ) {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      ...
      const index = record ? parseRecord(record) : {
        schemaVersion: 'tenant-fork-index-v1',
        tenantId: request.tenantId,
        entries: [],
        updatedAt: state.updatedAt
      };
      if (index.tenantId !== request.tenantId) {
        throw new ForkStateError('fork_identity_mismatch', 'Tenant index identity does not match the immutable request');
      }
      const entry = {
        forkId: state.forkId,
      ...
      index.entries.sort((a, b) => a.forkId.localeCompare(b.forkId));
      index.updatedAt = state.updatedAt;
      try {
  ...
  async createFork(input) {
    ...
    const requestedState = validateForkState({
      ...
      lastTransitionId: `create:${request.idempotencyKey}:requested`,
      lastFromState: 'none'
    });

    try {
      await this.storage.put(
        forkCurrentKey(request.forkId),
        canonicalJson(requestedState),
    ...
    if (current.requestDigest !== requestDigest) {
      throw new ForkStateError('fork_conflict', 'Fork ID already belongs to another request');
    }
    ...
    if (current.state === 'requested') {
      current = await this.transitionFork({
        forkId: request.forkId,
        tenantId: request.tenantId,
        attemptId: request.attemptId,
        from: 'requested',
      ...
      await this.ensureTransitionEvent(current);
      await this.#upsertTenantForkState(current);
    } else if (current.state !== targetState) {
      throw new ForkStateError('fork_conflict', 'Existing fork state does not match the immutable create request');
    }
    return current;
  }
  ...
  async ensureTransitionEvent(state) {
    const event = validateForkEvent({
      schemaVersion: 'fork-event-v1',
      eventId: eventId(state.version, state.lastTransitionId),
      forkId: state.forkId,
      tenantId: state.tenantId,
      attemptId: state.attemptId,
      requestDigest: state.requestDigest,
      from: state.lastFromState,
      to: state.state,
  ...
  async transitionFork(input) {
    const keys = new Set([
      'forkId', 'tenantId', 'attemptId', 'from', 'to', 'expectedEtag',
      'transitionId', 'occurredAt', 'blockNumber', 'blockHash'
    ]);
    if (!input || typeof input !== 'object' || Array.isArray(input)) {
      throw new ForkStateError('invalid_transition', 'Transition must be an object');
    }
    for (const key of Object.keys(input)) {
      if (!keys.has(key)) throw new ForkStateError('unknown_field', `Unknown transition field ${key}`);
    }
    ...
    if (current.lastTransitionId === input.transitionId) {
      await this.ensureTransitionEvent(current);
      await this.#upsertTenantForkState(current);
      return current;
    }
    if (current.etag !== input.expectedEtag || current.state !== input.from) {
      throw new ForkStateError('stale_state', 'Current state changed');
    }
    if (!ALLOWED_TRANSITIONS[current.state]?.has(input.to)) {
      throw new ForkStateError('invalid_transition', `${current.state} cannot transition to ${input.to}`);
    }
    ...
    const nextInput = {
      ...
      lastFromState: current.state,
      blockNumber: input.blockNumber ?? current.blockNumber,
      blockHash: input.blockHash ?? current.blockHash,
      ...(input.to === 'deleted' ? { deletedAt: input.occurredAt, tombstone: true } : {})
    };
    ...
    let written;
    try {
      written = await this.storage.put(
        forkCurrentKey(input.forkId),
        canonicalJson(next),
    ...
    const result = Object.freeze({ ...next, etag: written.etag });
    await this.#upsertTenantForkState(result);
    return result;
  }
  ...
  async publishActionResult(input) {
    ...
    if (current.state !== 'ready') {
      throw new ForkStateError('fork_not_ready', 'Fork is not ready for inert action results');
    }
  ...
  capability() {
    return FREE_DEVELOPMENT_FORK_CAPABILITY;
  }
}
