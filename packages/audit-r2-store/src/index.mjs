...
// Compatibility adapter for the original backend-injected store contract.
// It preserves the hardened conditional-write semantics while delegating bytes
// and persistence to an explicitly supplied backend.
export class AuditR2Store {
  constructor(backend) {
    if (!backend || typeof backend.head !== 'function' || typeof backend.get !== 'function' || typeof backend.put !== 'function' || typeof backend.delete !== 'function') {
      throw new TypeError('AuditR2Store requires a head/get/put/delete backend');
    }
  ...
  async put(key, value, options = {}) {
    if (typeof key !== 'string' || key.length < 1) throw new TypeError('R2 key is required');
    const previous = await this.backend.head(key);
    const condition = options.onlyIf ?? {};
  ...
  head(key) { return this.backend.head(key); }
  delete(key) { return this.backend.delete(key); }
}
