...
export class AuditR2Store {
  ...
  async put(key, value, options = undefined) {
    ...
    return this.backend.put(checkedKey, value);
  }

  get(key) { return this.backend.get(safeKey(key)); }
  head(key) { return this.backend.head(safeKey(key)); }
  delete(key) { return this.backend.delete(safeKey(key)); }
}
