...
- `serializePhase4ToolResultDocumentation()` serializes the documentation deterministically. Its byte-identical committed snapshot is `schemas/phase4-tool-result-contract-v2.schema.json`.
...
The runtime validator remains authoritative. The schema snapshot documents custom runtime-only behavior with `x-curveyield-*` keywords and never enables execution or silently broadens the accepted contract.
