...
function cloneEntries(value, name) {
  return array(value, name).map((entry, index) => structuredClone(object(entry, `${name}[${index}]`)));
}
...
export function normalizeControllerProjectionV3(value) {
  ...
  if (active.length !== 2 || active[0] !== 'ethereum' || active[1] !== 'base' || networkScope.default !== 'base') {
  ...
  if (provenance.eventHead !== null && provenance.eventHead !== undefined) exactSha(provenance.eventHead, 'provenance.eventHead');
  ...
  return {
    ...
    phases: cloneEntries(value.phases, 'phases'),
    lanes: cloneEntries(value.lanes, 'lanes'),
    workers: cloneEntries(value.workers, 'workers'),
    assignments: cloneEntries(value.assignments, 'assignments'),
    instructionProofs,
    findings: cloneEntries(value.findings, 'findings'),
    remediation: cloneEntries(value.remediation, 'remediation'),
    evidence: cloneEntries(value.evidence, 'evidence'),
    provenance: { eventCount: provenance.eventCount, eventHead: provenance.eventHead ?? null, recentEvents },
