...
export function utf8Bytes(value){return encoder.encode(value);}
export function utf8ByteLength(value){return utf8Bytes(value).byteLength;}
export function sha256HexFromBytes(input){
 const length=input.byteLength,paddedLength=((length+72)>>6)<<6,bytes=new Uint8Array(paddedLength);bytes.set(input);bytes[length]=0x80;
 const bits=length*8,view=new DataView(bytes.buffer);view.setUint32(paddedLength-8,Math.floor(bits/0x100000000),false);view.setUint32(paddedLength-4,bits>>>0,false);
...
export function sha256Text(value){return sha256HexFromBytes(utf8Bytes(value));}
