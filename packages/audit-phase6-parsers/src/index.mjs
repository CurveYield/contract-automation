...
function emptyResult(profileId, outcome = 'parser_error') {
  return {
    schemaVersion: 'formal-result-v1', profileId, outcome,
    obligations: [], assertions: [], models: [], traces: [], counterexamples: [],
...
function parserError(profileId, code, path = '$') {
  ...
  result.diagnostics.push({
    code: safeCode,
    severity: 'error',
    message: `Inert capture parsing failed with ${safeCode}`,
    sourceReferenceIds: []
  });
  ...
  try {
    ...
    return {
      ...
      diagnostics: [{ code: 'unexpected_parser_error', severity: 'error', message: 'Inert capture parsing failed with unexpected_parser_error', sourceReferenceIds: [] }],
      parserWarnings: [{ code: 'capture_rejected', message: 'Capture rejected at a bounded validation path', path: '$' }]
    };
  }
}
...
function stableRaw(value) {
  ...
  if (value && typeof value === 'object') {
    return `{${Object.keys(value).sort().map((key) => `${JSON.stringify(key)}:${stableRaw(value[key])}`).join(',')}}`;
  }
  ...
  for (const item of source) {
    ...
    if (previous) {
      if (previous.encoded !== encoded) {
        throw new Phase6ValidationError('conflicting_duplicate', `${path} contains conflicting duplicate identities`, path);
      }
      continue;
    }
...
function canonicalBoundedArray(source, key, limit, path) {
  const records = deduplicateRawByIdentity(source, (item) => rawIdentity(key, item), path)
    .sort((a, b) => a.identity.localeCompare(b.identity) || a.encoded.localeCompare(b.encoded));
  return {
    items: records.slice(0, limit).map(({ item }) => structuredClone(item)),
    truncated: records.length > limit
  };
}

function canonicalBoundedNestedArray(source, identityOf, limit, path) {
  const records = deduplicateRawByIdentity(source, identityOf, path)
    .sort((a, b) => a.identity.localeCompare(b.identity) || a.encoded.localeCompare(b.encoded));
  return {
    items: records.slice(0, limit).map(({ item }) => structuredClone(item)),
    truncated: records.length > limit
...
function boundedCaptureToResult(capture, profileId) {
  if (capture === null || typeof capture !== 'object' || Array.isArray(capture) || ![Object.prototype, null].includes(Object.getPrototypeOf(capture))) {
    throw new Phase6ValidationError('invalid_plain_object', 'capture must be an ordinary or null-prototype object', '$');
  }
  for (const key of Object.keys(capture)) {
    if (!CAPTURE_KEYS.has(key)) throw new Phase6ValidationError('unknown_field', 'capture contains a rejected field', '$.[rejected-field]');
  }
  for (const key of CAPTURE_KEYS) {
    if (!(key in capture)) throw new Phase6ValidationError('missing_field', `capture is missing ${key}`, `$.${key}`);
  }
...
function parseBytes(bytes, profileId) {
  ...
  try {
    ...
    catch { return parserError(profileId, 'invalid_utf8'); }
    let capture;
    try { capture = JSON.parse(text); }
    catch { return parserError(profileId, 'invalid_json'); }
