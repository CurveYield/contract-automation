...
export function assertPlainObject(value, path) {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new Phase6ValidationError('invalid_type', `${path} must be an object`, path);
  }
  return value;
}
...
export function assertAllowedKeys(value, allowed, path) {
  for (const key of Object.keys(value)) {
    if (!allowed.has(key)) {
      throw new Phase6ValidationError('unknown_field', `${path}.${key} is not allowed`, `${path}.${key}`);
    }
  }
}
...
export function assertRequiredKeys(value, required, path) {
  for (const key of required) {
    if (!(key in value)) {
      throw new Phase6ValidationError('missing_field', `${path}.${key} is required`, `${path}.${key}`);
    }
  }
}
...
export function assertString(value, path, maxLength = PHASE6_BOUNDS.shortStringChars, minLength = 1) {
  if (typeof value !== 'string') {
    throw new Phase6ValidationError('invalid_type', `${path} must be a string`, path);
  }
  if (value.length < minLength) {
    throw new Phase6ValidationError('string_too_short', `${path} is too short`, path);
  }
  if (value.length > maxLength) {
    throw new Phase6ValidationError('string_too_long', `${path} exceeds ${maxLength} characters`, path);
  }
  return value;
}
...
export function assertBoolean(value, path) {
  if (typeof value !== 'boolean') {
    throw new Phase6ValidationError('invalid_type', `${path} must be a boolean`, path);
  }
  return value;
}
...
export function assertEnum(value, allowed, path) {
  if (typeof value !== 'string' || !allowed.includes(value)) {
    throw new Phase6ValidationError('invalid_enum', `${path} is not an allowed value`, path);
  }
  return value;
}
...
export function assertIdentifier(value, path) {
  ...
  if (!/^[A-Za-z][A-Za-z0-9_.:-]*$/.test(value)) {
    throw new Phase6ValidationError('invalid_identifier', `${path} is not a valid identifier`, path);
  }
  return value;
}
...
export function assertStringArray(value, path, maxItems, maxChars = PHASE6_BOUNDS.shortStringChars, unique = true) {
  if (!Array.isArray(value) || value.length > maxItems) {
    throw new Phase6ValidationError('invalid_collection', `${path} must contain at most ${maxItems} items`, path);
  }
  const seen = new Set();
  return value.map((item, index) => {
    const checked = assertString(item, `${path}[${index}]`, maxChars);
    if (unique && seen.has(checked)) {
      throw new Phase6ValidationError('duplicate_item', `${path}[${index}] duplicates an earlier item`, `${path}[${index}]`);
    }
    seen.add(checked);
    return checked;
  });
...
export function scanPhase6ForbiddenFields(value, path = '$', depth = 0) {
  if (depth > PHASE6_BOUNDS.nestedCollectionDepth) {
    throw new Phase6ValidationError('nested_too_deep', `${path} exceeds the nested collection depth`, path);
  }
  ...
  for (const [key, child] of Object.entries(value)) {
    if (FORBIDDEN_KEYS.has(normalizedKey(key))) {
      throw new Phase6ValidationError('forbidden_field', `${path}.${key} is forbidden`, `${path}.${key}`);
    }
    scanPhase6ForbiddenFields(child, `${path}.${key}`, depth + 1);
  }
}
