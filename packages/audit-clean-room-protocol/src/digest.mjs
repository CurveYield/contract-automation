...
export function sha256HexFromBytes(input){
  const length=input.byteLength,paddedLength=((length+9+63)>>6)<<6,bytes=new Uint8Array(paddedLength);
  bytes.set(input);bytes[length]=0x80;const bits=length*8,view=new DataView(bytes.buffer);
...
export function sha256Hex(value){ return sha256HexFromBytes(utf8Bytes(value)); }
