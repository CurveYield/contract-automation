...
interface IERC20RewardConverterV26 {
...
interface ISdYbYbPoolRewardV26 {
    function coins(uint256 index) external view returns (address);
...
library SafeTokenRewardConverterV26 {
    error TokenCallFailed(address token);

    function safeTransfer(IERC20RewardConverterV26 token, address to, uint256 amount) internal {
        _call(token, abi.encodeCall(token.transfer, (to, amount)));
    }

    function safeTransferFrom(
        IERC20RewardConverterV26 token,
        address from,
        address to,
        uint256 amount
    ) internal {
    ...
    function forceApprove(IERC20RewardConverterV26 token, address spender, uint256 amount) internal {
    ...
    function _call(IERC20RewardConverterV26 token, bytes memory data) private {
...
contract CurveYieldSdYBRewardConverterV26 {
    string public constant converterVersion = "v26";

    using SafeTokenRewardConverterV26 for IERC20RewardConverterV26;
    ...
    IERC20RewardConverterV26 private immutable crvUsdToken;
    IERC20RewardConverterV26 private immutable ybToken;
    IERC20RewardConverterV26 private immutable sdYbToken;
    IYbCrvUsdPoolV26 public immutable ybCrvUsdPool;
    ISdYbYbPoolRewardV26 private immutable mainPool;
    ...
    constructor(address ybCrvUsdPool_) {
        ...
        IYbCrvUsdPoolV26 rewardPool = IYbCrvUsdPoolV26(ybCrvUsdPool_);
        ISdYbYbPoolRewardV26 fixedPool = ISdYbYbPoolRewardV26(SDYB_YB_POOL);
        if (
        ...
        crvUsdToken = IERC20RewardConverterV26(CRVUSD);
        ybToken = IERC20RewardConverterV26(YB);
        sdYbToken = IERC20RewardConverterV26(SDYB);
