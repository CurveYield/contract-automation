...
function raceHarness({failAt=null}={}){
  ...
  const transport={
    ...
    async applyLedgerMutation({mutation}){
      ...
      if(mutation.operation==='update-cas'&&((!existing&&mutation.expectedBlobSha!=='0'.repeat(40))||(existing&&existing.blobSha!==mutation.expectedBlobSha))){
        const e=new Error('stale_blob_sha');e.code='stale_blob_sha';throw e;
      }
      const blobSha=mutation.nextContentBlobSha;store.set(mutation.path,{content:mutation.content,blobSha});return {applied:true,nextBlobSha:blobSha};
    },
    async getPublication({kind,idempotencyKey}){return publications.get(`${kind}:${idempotencyKey}`)??null},
    async publish(plan){if(plan.kind===failPublishKind&&!publishFailed){publishFailed=true;const e=new Error('transport');e.status=503;throw e}const key=`${plan.kind}:${plan.idempotencyKey}`;const prior=publications.get(key);if(prior&&JSON.stringify(prior)!==JSON.stringify(plan)){const e=new Error('publication_conflict');e.code='publication_conflict';throw e}if(prior)return {action:'noop'};publications.set(key,plan);return {published:true,publicationId:plan.publicationId}},
