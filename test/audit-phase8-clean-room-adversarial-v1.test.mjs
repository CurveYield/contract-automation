...
import {
  createCleanRoomPolicy, validateCleanRoomPolicy, createCampaignAccessContext, validateCampaignAccessContext,
  createShareGrant, validateShareGrant, createShareGrantRevocation, validateShareGrantRevocation,
  canonicalClone
...
import {
  createMergeRequest, validateMergeRequest, createInitialMergeState, validateMergeState, transitionMergeState, validateMergeEvent,
  buildRelationMaps, validateDuplicateRelation, validateConflictRelation, createMergeManifest, validateMergeManifest
...
import {
  ...
  createProvenanceIndex, validateProvenanceIndex, createMergedReportReference, validateMergedReportReference
...
function invalidFor(value,key){
  if(Array.isArray(value))return null;if(value===null)return '*';if(typeof value==='boolean')return 'false';if(typeof value==='number')return -0;
...
const contracts=[
  ['policy',policy,validateCleanRoomPolicy],['context',context,validateCampaignAccessContext],['grant',grant,validateShareGrant],['revocation',revocation,validateShareGrantRevocation],
  ['accessDecision',accessDecision,validateAccessDecision],['visibility',visibility,validateVisibilityDecision],['terminal',ta,validateTerminalCampaignManifest],['mergeRequest',request,validateMergeRequest],
  ['mergeState',state,validateMergeState],['mergeEvent',event,validateMergeEvent],['duplicateRelation',maps.duplicateRelations[0],validateDuplicateRelation],['conflictRelation',maps.conflictRelations[0],validateConflictRelation],
  ['provenanceNode',source,validateProvenanceNode],['provenanceEdge',edge,validateProvenanceEdge],['provenanceIndex',provenance,validateProvenanceIndex],['reportReference',report,validateMergedReportReference],['mergeManifest',mergeManifest,validateMergeManifest]
];

test('one-field invalid mutations are rejected across every public output contract',()=>{
  let mutationCount=0;
  ...
  assert.equal(mutationCount,176);
});
...
test('hostile reflection corpus returns bounded errors without executing accessors',()=>{
  ...
  const symbol={};symbol[Symbol('hidden')]=1;assert.throws(()=>createCleanRoomPolicy(symbol),(error)=>error.code==='symbol_field'||error.code==='missing_field');
  const sparse=['campaign:read'];sparse.length=2;assert.throws(()=>createCleanRoomPolicy({...policy,allowedScopes:sparse}),{code:'unknown_field'});
...
test('numeric, string, path, collection, and substitution corpus is rejected deterministically',()=>{
  for(const bad of [-0,NaN,Infinity,Number.MAX_SAFE_INTEGER+1])assert.throws(()=>createCleanRoomPolicy({...policy,maxCampaigns:bad}), (error)=>typeof error.code==='string');
  assert.throws(()=>createCampaignAccessContext({...context,requesterId:'user\u0000secret'}),(error)=>error.code==='unknown_field'||error.code==='control_character');
  ...
  assert.throws(()=>createCleanRoomPolicy({...policy,allowedScopes:Array.from({length:100},()=> 'campaign:read')}),(error)=>typeof error.code==='string');
  assert.throws(()=>createMergeRequest({terminalManifests:[ta,createTerminalCampaignManifest({...structuredClone(tb),tenantId:'tenant-b'})],policyId:policy.policyId,requestedBy:'user-a',requestedAt:ts,idempotencyKey:'request-b',expectedCurrentEtag:d('e')}),(error)=>typeof error.code==='string');
});
